...
var _errorParser = require('./errorParser');

var _errorParser2 = _interopRequireDefault(_errorParser);

var _joiErrorCatalog = require('./joiErrorCatalog');

var _joiErrorCatalog2 = _interopRequireDefault(_joiErrorCatalog);
...
let buildJoiError = error => {
  ...
  let joiError = _joiErrorCatalog2.default[errorType];
  if (!joiError) {
    return (0, _errorParser2.default)(10000, `- Error code not found for joi error ${errorType}`);
  }

  let parameters = joiError.parameters(errorDetails[0], [parameterType, parameterPath]);

  return (0, _errorParser2.default)(joiError.code, parameters);
};

let buildGenericErrors = error => {
  if (error.output.statusCode === 401) {
    return (0, _errorParser2.default)(30001, 'make sure the header parameter Authorization is valid');
  }

  return (0, _errorParser2.default)(10000, `- Error code not found for joi error ${error.output.payload.message}`);
};

exports.default = { buildJoiError, buildGenericErrors };
//# sourceMappingURL=joiErrorParser.js.map
